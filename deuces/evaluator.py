...
class Evaluator(object):
    ...
    def evaluate_three(self, cards, board):
        """
        evaluate three card board ,only able to form triplet and pair,or junk
        use different evaluation score system
        """
        cards = cards + board
        assert(len(cards) == 3)
        ...
        if len(set(cards)) == 1:
            ...
            if cards[0] == cards[1]:
                ...
                pair_card = cards[0]
                junk_card = cards[1]
            return 13 + pair_card * 13 + junk_card
        elif len(set(cards)) == 3:
            return 182 + cards[0] * 169 + cards[1] * 13 + cards[2]
